...
import Marketplace, { isOwned, SHOP_ITEMS } from './Marketplace.jsx';
import ChatSkin from './ChatSkin.jsx';
import EmailSkin from './EmailSkin.jsx';
import DartSkin from './DartSkin.jsx';
import { AuthModal, useAuth } from './Auth.jsx';
...
const EFFECTS = [
  ...
  { id: 'shh',   label: '🔇 RIP',   free: true,  desc: 'ฉีกทิ้งเลย' },
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        gap: 10, padding: 24, backdropFilter: 'blur(2px)',
      }}
    >
...
function AppInner() {
  ...
  const { t } = useLang();
  const [mainTab, setMainTab]   = useState('bin');
  ...
  const runBin = () => {
    playSound('paper trash.mp3');
    const W = window.innerWidth, H = window.innerHeight;
    const sx = -30, sy = H * 0.38;
    const binCX = W * 0.72, binCY = H * 0.58;
    const tx = binCX - 24, ty = binCY - 24;
    setBinState(s => ({ ...s, visible: true, paperX: sx, paperY: sy, paperScale: 1, paperRot: 0, paperShow: true, tntShow: false, lidRot: 0, lidY: 0, binScaleX: 1, binScaleY: 1 }));
    setTimeout(() => setBinState(s => ({ ...s, lidRot: -28, lidY: -6 })), 300);
    animBezier(sx, sy, W * 0.3, sy - H * 0.2, tx, ty, 700,
      (x, y, e) => setBinState(s => ({ ...s, paperX: x, paperY: y, paperScale: 1 - e * 0.5, paperRot: e * 400 })),
      () => {
  ...
  const runBoom = () => {
    ...
    const binCX = W * 0.72, binCY = H * 0.58;
    const tx = binCX - 24, ty = binCY - 24;
    setBinState(s => ({ ...s, visible: true, paperX: sx, paperY: sy, paperScale: 1, paperRot: 0, paperShow: true, tntShow: false, lidRot: 0, lidY: 0, binScaleX: 1, binScaleY: 1, smokeList: [] }));
    setTimeout(() => setBinState(s => ({ ...s, lidRot: -28, lidY: -6 })), 300);
    animBezier(sx, sy, W * 0.3, sy - H * 0.2, tx, ty, 700,
      (x, y, e) => setBinState(s => ({ ...s, paperX: x, paperY: y, paperScale: 1 - e * 0.5, paperRot: e * 400 })),
      () => {
  ...
  const runFade = () => {
    ...
    const chars = [...content].map((ch, i) => ({ id: i, char: ch, vanish: false, dx: (Math.random() - 0.5) * 10, dy: (Math.random() - 0.5) * 10 }));
    setFadeChars(chars);
    const shuffled = [...chars].sort(() => Math.random() - 0.5);
    const totalDur = 1800;
    shuffled.forEach((c, i) => {
      const delay = 400 + (i / shuffled.length) * totalDur + Math.random() * 150;
      setTimeout(() => { setFadeChars(prev => prev.map(p => p.id === c.id ? { ...p, vanish: true } : p)); }, delay);
    });
      ...
      <AnimatePresence>
        {binState.visible && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            {binState.paperShow && (
              <div style={{ position: 'absolute', left: binState.paperX, top: binState.paperY, width: 48, height: 48, transform: `scale(${binState.paperScale}) rotate(${binState.paperRot}deg)`, transformOrigin: 'center' }}>
            ...
            {binState.tntShow && (
              <div style={{ position: 'absolute', left: binState.tntX, top: binState.tntY, width: 44, height: 44, transform: `rotate(${binState.tntRot}deg)`, transformOrigin: 'center' }}>
            ...
            <div style={{ position: 'absolute', right: '10%', bottom: '16%', width: 90, height: 110 }}>
              <BinSVG lidRot={binState.lidRot} lidY={binState.lidY} scaleX={binState.binScaleX} scaleY={binState.binScaleY} style={{ width: '100%', height: '100%' }} />
            </div>
      ...
      <AnimatePresence>
        {fadeChars.length > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <div style={{ background: '#fffef5', border: '3px solid #1a1a2e', borderRadius: 8, boxShadow: '5px 5px 0 #1a1a2e', padding: '24px 20px', width: '72%', maxWidth: 300, position: 'relative' }}>
              <div style={{ position: 'absolute', left: 32, top: 0, bottom: 0, borderLeft: '2px solid #fca5a5' }} />
              <div style={{ fontFamily: "'Comic Neue',cursive", fontSize: 16, fontWeight: 700, color: '#1a1a2e', textAlign: 'center', lineHeight: 1.8, position: 'relative', zIndex: 1, wordBreak: 'break-word', display: 'flex', flexWrap: 'wrap', justifyContent: 'center' }}>
                {fadeChars.map(c =>
                  c.char === '\n'
      ...
      <div style={{ background: '#fef3c7', border: '5px solid #1a1a2e', borderRadius: 14, overflow: 'hidden', boxShadow: '8px 8px 0 #1a1a2e' }}>
        ...
        <div style={{ background: '#dc2626', borderBottom: '5px solid #1a1a2e', padding: '14px 18px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 5px 0 #7f1d1d' }}>
          <div>
            ...
            <div style={{ fontSize: 9, fontWeight: 700, color: '#fca5a5', letterSpacing: '1.5px', textTransform: 'uppercase', marginTop: 3, lineHeight: 1.4 }}>{t.tagline1}<br/>{t.tagline2}</div>
          </div>
        ...
        {/* MAIN NAV */}
        <div style={{ background: '#1a1a2e', borderBottom: '3px solid #1a1a2e', padding: '0', display: 'flex' }}>
          {[
            { id: 'bin',   label: t.nav_bin,   color: '#dc2626' },
            { id: 'chat',  label: t.nav_chat,  color: '#7c3aed', locked: !isOwned('skin_chat', purchases) },
            { id: 'email', label: t.nav_email, color: '#0891b2', locked: !isOwned('skin_email', purchases) },
            { id: 'dart',  label: t.nav_dart,  color: '#dc2626', locked: !isOwned('skin_dart', purchases) },
            { id: 'shop',  label: t.nav_shop,  color: '#16a34a' },
          ].map(nav => (
            <button key={nav.id} onClick={() => setMainTab(nav.id)}
              style={{
                flex: 1, fontFamily: "'Bangers',cursive", fontSize: 11,
                letterSpacing: 1, padding: '8px 2px',
                background: mainTab === nav.id ? nav.color : 'transparent',
                color: mainTab === nav.id ? '#fef3c7' : '#6b7280',
                border: 'none', borderBottom: mainTab === nav.id ? `3px solid ${nav.color}` : '3px solid transparent',
                cursor: 'pointer', transition: 'all .1s', position: 'relative',
              }}
            >
              {nav.label}
              {nav.locked && <span style={{ position: 'absolute', top: 2, right: 2, fontSize: 7 }}>🔒</span>}
            </button>
          ))}
        </div>
          ...
          <>
            <div style={{ background: dailyDone ? '#065f46' : '#15803d', borderBottom: '3px solid #1a1a2e', padding: '5px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{ fontFamily: "'Bangers',cursive", fontSize: 13, color: '#fef3c7', background: '#16a34a', border: '2px solid #1a1a2e', padding: '2px 10px', borderRadius: 3, boxShadow: '2px 2px 0 #1a1a2e', letterSpacing: 1 }}>
                {dailyDone ? t.daily_done_btn : t.daily_pts}
              </span>
            </div>
              ...
              <AnimatePresence mode="wait">
                {phase !== 'done' ? (
                  <motion.div key={`surface-${tab}-${phase}`} style={{ flex: 1, display: 'flex', flexDirection: 'column' }}
                    ...
                    {tab === 'voice' && (
                      <VoiceTab voice={voice} voiceText={voiceText} onClear={() => setVoiceText('')} t={t} />
                    )}
                  </motion.div>
                ) : (
            ...
            <div style={{ padding: '8px 14px 16px', display: 'flex', justifyContent: 'center' }}>
              ...
              >{t.dispose_btn}</motion.button>
            </div>
          </>
        )}

        {/* FOOTER */}
        <div style={{ background: '#1a1a2e', padding: '8px 14px', display: 'flex', flexDirection: 'column', gap: 6 }}>
          <a href="https://www.amazon.com/dp/B0GZZ3V7K3" target="_blank" rel="noopener noreferrer"
            style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#f59e0b', border: '2px solid #fbbf24', borderRadius: 6, padding: '7px 12px', textDecoration: 'none', boxShadow: '0 0 8px rgba(251,191,36,0.4)' }}>
            <span style={{ fontSize: 14 }}>📖</span>
            <div>
              <div style={{ fontFamily: "'Bangers',cursive", fontSize: 13, color: '#1a1a2e', letterSpacing: 2, lineHeight: 1 }}>{t.footer_book}</div>
              <div style={{ fontSize: 8, color: '#1a1a2e', fontWeight: 700, opacity: 0.7, letterSpacing: 1 }}>THE BIN: A Place to Let It Out · $9.99</div>
            </div>
            <span style={{ fontSize: 12, color: '#1a1a2e', fontWeight: 700 }}>→</span>
          </a>
...
function VoiceTab({ voice, voiceText, onClear, t }) {
  return (
      ...
      {!voice.supported ? (
        ...
        <>
          ...
          {voiceText
            ...
            : !voice.errMsg && <p style={{ fontSize: 11, color: '#6b7280', fontStyle: 'italic' }}>{voice.listening ? t.voice_listening : t.voice_tap}</p>
          }
